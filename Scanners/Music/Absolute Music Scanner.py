...
try:
    platform = sys.platform.lower()  # sys.platform: win32 | darwin | linux2,
except:
    platform = "other"
#  try:    platform = Platform.OS.lower()   # Platform.OS:  Windows, MacOSX, or Linux
#  except: p5latform = ""
if platform == "win32" or platform == "windows":
    LOG_PATH = os.path.expandvars("%LOCALAPPDATA%\\Plex Media Server\Logs")
...
logging.basicConfig(
    filename=os.path.join(LOG_PATH, LOG_FILE), format=LOG_FORMAT, level=logging.DEBUG
)
